...
- 起動後に原稿のファイルタイプを変更すると、ファイルを原稿ツリーで並べ替える時にファイル名が不正になるバグを修正しました

# 3.4.12

- HTMLのコメントが文字数に不正に登場しているのを修正しました
- VS Codeでエディターを選択した時に、原稿ツリーがアクティブになってしまう問題を解消しました
    - 原稿ツリーでファイル名を変更するときは、一度クリックしてからEnterキーを入力してください
- HTMLコメントが原稿用紙プレビューで正しく表示されるように修正しました

# 3.4.11

- PDF出力時のファイルパスを見直しました　Mac OS, Windows, WSL, Linux環境のいずれでもPDFの保存ができるようになっていると思います
    - なお、PDF出力にはNode.jsのインストールが必要です  以下のURLからダウンロードしてインストールしてからご利用ください
    [node.js](https://nodejs.org/)

# 3.4.10

- 3.4.9で行ったWSL対応にバグがあったため、一度処理を元に戻します

# 3.4.9

- WSL環境でPDF出力を行うときに、file:///を用いていたため保存ができなかったバグに対処しました
- ファイルの並び順を辞書ソートに変更しました
    - 今まで1-scene.txt, 11-scene.txt, 2-scene.txtの順に並んでいたファイルは、1-scene.txt, 2-scene.txt, 11-scene.txtの順に並ぶようになりました    

# 3.4.8

- 原稿ツリーの初期状態をフォルダーが閉じている状態に変更しました
- 原稿ツリーでフォルダーの描画を幾分か早くなるように改善しました
- novel-writerがワークスペースに保存している情報をリセットする「novel-writer情報リセット」コマンドを追加しました

# 3.6.7

- 原稿ツリーのフォルダーの開閉を記憶できるようになりました
- 原稿ツリーで選択したファイルやフォルダーを、Option, Altキーと矢印キーで上下の順番を入れ替えられるようになりました
